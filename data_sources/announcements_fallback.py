...
def fetch(code: str, limit: int = 20) -> Dict[str, Any]:
    ...
    if isinstance(payload, dict) and payload.get("_error"):
        errors.append(build_error("announcements_fallback", "exception", str(payload.get("_error"))))

    if not isinstance(items, list):
        errors.append(build_error("announcements_fallback", "invalid", "unexpected payload"))
        items = []
    ...
    if not announcements:
        errors.append(build_error("announcements_fallback", "empty", "no announcements returned"))

    meta = {
        "source": "announcements_fallback",
